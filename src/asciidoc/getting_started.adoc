...
GPars as a library depends on Groovy version equal to or greater than 2.0. Also, the Fork/Join concurrency
library must be available. This comes as standard with Java 7, but not with Java 6. Groovy currently caters
for Java 6 and so GPars is dependent on the http://gee.cs.oswego.edu/dl/jsr166/dist/jsr166y.jar[jsr166y]
artefact (an artefact from the http://g.oswego.edu/dl/concurrency-interest/[JSR-166 Project]). This artefact
must be on the classpath of programs which use current released versions of GPars to compile and execute.
Released versions of the jsr166y artefact are in the main Maven and Codehaus repositories.  Development versions of
the artefact are available in the Codehaus snapshots repository.  Using Gradle or Grapes you would use the
following dependency specification:
----
...
Future versions of GPars 1.x will depend on Java 7 and avoid this dependency on additional artefacts. GPars
2.x will depend on Java 8 and will only be usable with Groovy 2.3.0 and later.
...
Check out the http://jira.codehaus.org/secure/ReleaseNote.jspa?projectId=12030&version=19439[JIRA release notes]
