...
/**
 * This class forms the core of the DSL initialized by <i>ParallelCollections</i>. The static methods of <i>ParallelArrayUtil</i>
 * get attached to their first arguments (the Groovy Category mechanism) and can be then invoked as if they were part of
 * the argument classes.
 * @see groovyx.gpars.ParallelCollections
 *
 * @author Vaclav Pech
...
public class ParallelArrayUtil {
    ...
    private static ForkJoinPool retrievePool() {
        final ForkJoinPool pool = groovyx.gpars.ParallelCollections.retrieveCurrentPool()
    ...
    public static Object makeTransparent(Object collection) {
        if (!(collection.respondsTo('isTransparent'))) throw new IllegalStateException("Cannot make the object transparently parallel. Apparently we're not inside a ParallelCollections.withPool() block nor the collection hasn't been enhanced with ParallelEnhancer.enhance().")
     ...
     * Example:
     * ParallelCollections.withPool {*     def result = new ConcurrentSkipListSet()
     ...
     * Example:
     * ParallelCollections.withPool {*     def result = [1, 2, 3, 4, 5].collectParallel {Number number -> number * 10}*     assertEquals(new HashSet([10, 20, 30, 40, 50]), result)
     *}*/
     ...
     * Example:
     * ParallelCollections.withPool {*     def result = [1, 2, 3, 4, 5].findAllParallel {Number number -> number > 3}*     assertEquals(new HashSet([4, 5]), result)
     *}*/
     ...
     * Example:
     * ParallelCollections.withPool {*     def result = [1, 2, 3, 4, 5].findParallel {Number number -> number > 3}*     assert (result in [4, 5])
     *}*/
     ...
     * Example:
     * ParallelCollections.withPool {*     def result = [1, 2, 3, 4, 5].grepParallel(4..6)
     ...
     * have a new <i>grepParallel(Closure cl)</i> method, which delegates to the <i>ParallelArrayUtil</i> class.
     * Example:
     * ParallelCollections.withPool {*     def result = [1, 2, 3, 4, 5].splitParallel(it > 3)
     ...
     * have a new <i>grepParallel(Closure cl)</i> method, which delegates to the <i>ParallelArrayUtil</i> class.
     * Example:
     * ParallelCollections.withPool {*     def result = [1, 2, 3, 4, 5].splitParallel(4..6)
     *            assert [3, 4, 5] as Set == result[0] as Set
     *            assert [1, 2] as Set == result[1] as Set
     *}*/
     ...
     * have a new <i>grepParallel(Closure cl)</i> method, which delegates to the <i>ParallelArrayUtil</i> class.
     * Example:
     * ParallelCollections.withPool {*     def result = [1, 2, 3, 4, 5].countParallel(4)
     ...
     * Example:
     * ParallelCollections.withPool {*     assert [1, 2, 3, 4, 5].anyParallel {Number number -> number > 3}*     assert ![1, 2, 3].anyParallel {Number number -> number > 3}*}*/
     ...
     * Example:
     * ParallelCollections.withPool(5) {*     assert ![1, 2, 3, 4, 5].everyParallel {Number number -> number > 3}*     assert [1, 2, 3].everyParallel() {Number number -> number <= 3}*}*/
     ...
     * Example:
     * ParallelCollections.withPool {*     assert ([1, 2, 3, 4, 5].groupByParallel {Number number -> number % 2}).size() == 2
     *}*/
