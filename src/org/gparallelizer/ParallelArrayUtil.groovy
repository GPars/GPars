...
public class ParallelArrayUtil {
    ...
    private static <T> ParallelArray createPA(Collection<T> collection, ForkJoinExecutor pool) {
        return ParallelArray.createFromCopy(collection.toArray(new T[collection.size()]), pool)
    }

    static java.util.Collection<Object> createCollection(Object object) {
    ...
    public static <T> Collection<T> eachAsync(Collection<T> collection, Closure cl) {
        createPA(collection, retrievePool()).withMapping({cl(it)} as Op).all()
        return collection
    }
    ...
    public static <T> Collection<T> collectAsync(Collection<T> collection, Closure cl) {
        createPA(collection, retrievePool()).withMapping({cl(it)} as Op).all().asList()
    }
     ...
     * }
     */
    public static <T> T findAsync(Collection<T> collection, Closure cl) {
